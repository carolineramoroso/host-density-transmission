#Code that accompanies Antonovics, Amoroso, Bruns, & Hood 
#"Host density shapes the relative contribution of vector-based and aerial
#transmission of a pathogenic fungus"
#Ecology
...
#distance from the source of spores (in meters) and the number of spores deposited 
#on flowers is mediated by plant density. - with a subset of distances
...
# table s7 - both experiments combined, consistent with analyses separately
anova(lm(log10(spores+1)~ nsources + Experiment,  
         data=aerial_Donly))
summary(lm(log10(spores+1)~ nsources + Experiment,  
           data=aerial_Donly))
...
plot_smooths(model = polltypes_gam1, series = Spacing, comparison = Pollinator) + 
  geom_jitter( aes(x=Spacing, y=log10(Visits_per_tube+0.027), color=Pollinator), 
               alpha=0.7, data=poll_data_sub_gat)+
  ylab("Log10(Primary visits per plant+0.027)") + scale_x_reverse() +
  xlab("Spacing (m)") +
  scale_color_manual(values=c("#4daf4a","#984ea3","#e41a1c","#377eb8")) + 
  scale_fill_manual(values=c("#4daf4a","#984ea3","#e41a1c","#377eb8")) +
